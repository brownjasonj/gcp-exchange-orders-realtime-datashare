resource "google_storage_bucket" "dataflow_temp_bucket" {
  name                        = "${var.project_id}-dataflow-temp-mergeview"
  location                    = var.region
  force_destroy               = true
...
resource "google_dataflow_flex_template_job" "pricing_to_bq_job" {
  ...
  name                    = "pricing-to-bq-job-mergeview"
  ...
  parameters = {
    ...
    outputTableSpec   = "${var.project_id}:${google_bigquery_table.bidask_all_staged.dataset_id}.${google_bigquery_table.bidask_all_staged.table_id}"
